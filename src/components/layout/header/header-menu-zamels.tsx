...
const HeaderMenu: React.FC<MenuProps> = ({ data, className }) => {
    ...
    return (
        <nav className={classNames(` flex w-full relative text-white`, className)}>
            {data?.map((item: any) =>
            (
                <div
                ...
                >
                    <Link
                        href={item.path}
                        className={
                            `uppercase relative inline-flex items-center px-3 py-0 pt-0 text-sm zamels-header-font xl:px-4 hover:text-black` +
